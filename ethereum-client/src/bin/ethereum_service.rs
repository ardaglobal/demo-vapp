...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::PublishState { state_id, state_root, proof, public_values } => {
            ...
            let result = client.publish_state_root(state_id, state_root, proof, public_values).await?;
        ...
        Commands::VerifyProof { proof, public_values } => {
            let proof = parse_bytes(&proof)?;
            let public_values = parse_bytes(&public_values)?;
        ...
        Commands::GetHistory { state_id, limit } => {
            ...
            let history = client.get_historical_states(state_id, limit).await?;
            
            println!("State history:");
            println!("State ID: {:?}", history.state_id);
            println!("Number of states: {}", history.state_roots.len());
            
            for (i, (((root, block), timestamp), proof_id)) in history.state_roots
                .iter()
                ...
                .enumerate()
            {
                println!("  {}: Root={:?}, Block={}, Time={}, Proof={:?}", 
                    i, root, block, timestamp, proof_id);
            }
        }
        ...
        Commands::NetworkStats => {
            ...
            println!("Sync status: {}", if stats.sync_status { "Synced" } else { "Not synced" });
        }
        
        Commands::CheckInclusion { leaf_hash, leaf_index, siblings, root } => {
            ...
            let proof = client.check_inclusion_proof(leaf_hash, leaf_index, siblings, root).await?;
        ...
        Commands::GetProofResult { proof_id } => {
            ...
            let result = client.get_proof_result(proof_id).await?;
            ...
            if let Some(result_bytes) = result {
                ...
                if result_bytes.len() == 4 {
                    let int_result = i32::from_be_bytes([result_bytes[0], result_bytes[1], result_bytes[2], result_bytes[3]]);
        ...
        Commands::GetProofData { proof_id } => {
            ...
            if let Some(proof_bytes) = proof_data {
                ...
                println!("  Proof hash: 0x{}", hex::encode(&alloy_primitives::keccak256(&proof_bytes)));
                
                // Don't print full proof data as it's very large
                println!("  First 64 bytes: 0x{}", hex::encode(&proof_bytes[..std::cmp::min(64, proof_bytes.len())]));
            } else {
        ...
        Commands::GetVerificationData { proof_id } => {
            ...
            info!("Retrieving complete verification data for {:?}...", proof_id);
            ...
            if let Some(data_bytes) = verification_data {
                ...
                println!("  Data hash: 0x{}", hex::encode(&alloy_primitives::keccak256(&data_bytes)));
                
                // TODO: Once proper VerificationData struct is implemented, decode the bytes properly
                println!("  Raw data: 0x{}", hex::encode(&data_bytes[..std::cmp::min(64, data_bytes.len())]));
            } else {
        ...
        Commands::GetStateProofHistory { state_id } => {
            ...
            let proof_ids = client.get_state_proof_history(state_id).await?;
        ...
        Commands::VerifyIndependently { proof_id } => {
            ...
            info!("Performing independent verification for proof {:?}...", proof_id);
            ...
            println!("  Verification Status: {}", if result.verified { "✅ PASSED" } else { "❌ FAILED" });
            println!("  Block Number: {}", result.block_number);
            ...
            // Summary
            println!("\n🎯 Overall Status: {}", 
                if result.verified { "✅ VERIFIED" } else { "❌ VERIFICATION FAILED" });
        ...
        Commands::GetVerifierVersion => {
            ...
            let version = client.get_verifier_version().await?;
        ...
        Commands::TrustlessVerify { proof_id, save_to_file } => {
            let proof_id = parse_bytes32(&proof_id)?;
            ...
            let trustless_summary = ethereum_client::types::TrustlessVerificationSummary {
                ...
                verification_details: format!("Verified: {}, Block: {}, Gas: {}", 
            ...
            println!("==================================");
            println!("Status: {}", match trustless_summary.verification_status {
            ...
            println!("Independent Verification: {}", 
                if trustless_summary.independent_verification_passed { "✅ PASSED" } else { "❌ FAILED" });
            println!("Details: {}", trustless_summary.verification_details);
            
            // Save to file if requested
            if save_to_file {
                let filename = format!("trustless_verification_{}.json", hex::encode(&proof_id[..8]));
...
fn parse_bytes32(s: &str) -> Result<alloy_primitives::FixedBytes<32>> {
    ...
    if bytes.len() != 32 {
        return Err(ethereum_client::EthereumError::General(
            eyre::eyre!("Expected 32 bytes, got {}", bytes.len())
        ));
    }
...
fn format_check(passed: bool) -> String {
