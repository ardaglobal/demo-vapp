...
impl EthereumCache {
    ...
    pub async fn get_state_updates_by_state_id(
        &self,
        state_id: StateId,
        limit: Option<i64>,
        ...
        for row in rows {
            updates.push(StateUpdate {
                ...
                public_values: alloy_primitives::Bytes::from(
                    row.get::<Vec<u8>, _>("public_values"),
                ),
                block_number: row.get::<Option<i64>, _>("block_number").map(|n| n as u64),
                transaction_hash: row
                    .get::<Option<&[u8]>, _>("transaction_hash")
        ...
        row.map_or_else(
            ...
            |row| {
                Ok(Some(ProofSubmission {
                    ...
                    submitter: alloy_primitives::Address::from_slice(
                        row.get::<&[u8], _>("submitter"),
                    ),
                    block_number: row.get::<i64, _>("block_number") as u64,
                    transaction_hash: alloy_primitives::FixedBytes::from_slice(
                        row.get::<&[u8], _>("transaction_hash"),
                    ),
    ...
    pub async fn get_events_by_block_range(
        ...
        for row in rows {
            events.push(ContractEvent {
                ...
                state_id: row
                    .get::<Option<&[u8]>, _>("state_id")
                    .map(|s| StateId::from_slice(s)),
                proof_id: row
                    .get::<Option<&[u8]>, _>("proof_id")
        ...
        row.map_or_else(
            ...
            |row| {
                Ok(Some(NetworkStats {
                    ...
                    block_number: row.get::<i64, _>("block_number") as u64,
                    gas_price: row
                        .get::<String, _>("gas_price")
                        .parse()
                        .unwrap_or_default(),
                    base_fee: row
                        .get::<Option<String>, _>("base_fee")
    ...
    pub async fn cleanup_old_data(&self, days_to_keep: i32) -> Result<u64> {
        ...
        info!(
            "Cleaned up {} old events and {} old network stats",
            deleted_events, deleted_stats
        );
