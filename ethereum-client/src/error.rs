    ...
    pub fn from_contract_error(error_msg: &str) -> Self {
        ...
        if let Some(sig_start) = error_msg.find("0x") {
            ...
            if sig_end <= error_msg.len() {
                ...
                match signature {
                    "0x344fd586" => return Self::UnauthorizedAccess,
                    "0x9d89020a" => return Self::InvalidArrayLength,
                    "0xfa8d84c7" => return Self::ContractStateNotFound,
                    "0x36131e57" => return Self::ContractProofNotFound,
        ...
        Self::Contract(error_msg.to_string())
    }
}
...
mod tests {
    ...
    fn test_parse_unauthorized_access_error() {
        ...
        match parsed_error {
            super::EthereumError::UnauthorizedAccess => {}
    ...
    fn test_parse_invalid_array_length_error() {
        ...
        match parsed_error {
            super::EthereumError::InvalidArrayLength => {}
    ...
    fn test_parse_state_not_found_error() {
        ...
        match parsed_error {
            super::EthereumError::ContractStateNotFound => {}
    ...
    fn test_parse_proof_not_found_error() {
        ...
        match parsed_error {
            super::EthereumError::ContractProofNotFound => {}
    ...
    fn test_parse_invalid_limit_error() {
        ...
        match parsed_error {
            super::EthereumError::InvalidLimit => {}
    ...
    fn test_parse_invalid_index_error() {
        ...
        match parsed_error {
            super::EthereumError::InvalidIndex => {}
    ...
    fn test_parse_unknown_error_signature() {
        ...
        match parsed_error {
            super::EthereumError::Contract(msg) => {
    ...
    fn test_parse_error_without_signature() {
        ...
        match parsed_error {
            ...
            _ => panic!(
                "Expected Contract error for message without signature, got: {parsed_error:?}"
            ),
        }
    }
    ...
    fn test_parse_multiple_signatures_uses_first() {
        ...
        match parsed_error {
            super::EthereumError::UnauthorizedAccess => {}
            _ => {
                panic!("Expected UnauthorizedAccess error (first signature), got: {parsed_error:?}")
            }
        }
    }
    ...
    fn test_error_message_display() {
        ...
        let unauthorized = super::EthereumError::UnauthorizedAccess;
        assert_eq!(
            unauthorized.to_string(),
            "Unauthorized access: The caller is not authorized to perform this operation"
        );

        let invalid_array = super::EthereumError::InvalidArrayLength;
        assert_eq!(
            invalid_array.to_string(),
            "Invalid array length: Input arrays have mismatched lengths"
        );

        let state_not_found = super::EthereumError::ContractStateNotFound;
        assert_eq!(
            state_not_found.to_string(),
            "State not found: The requested state does not exist"
        );

        let proof_not_found = super::EthereumError::ContractProofNotFound;
        assert_eq!(
            proof_not_found.to_string(),
            "Proof not found: The requested proof does not exist"
        );

        let invalid_limit = super::EthereumError::InvalidLimit;
        assert_eq!(
            invalid_limit.to_string(),
            "Invalid limit: The provided limit parameter is invalid"
        );

        let invalid_index = super::EthereumError::InvalidIndex;
        assert_eq!(
            invalid_index.to_string(),
            "Invalid index: The provided index is out of bounds"
        );

        let proof_exists = super::EthereumError::ProofAlreadyExists;
        assert_eq!(
            proof_exists.to_string(),
            "Proof already exists: A proof with this ID already exists"
        );
    }
    ...
    fn test_case_insensitive_signature_parsing() {
        ...
        match parsed_error {
            super::EthereumError::Contract(_) => {}
