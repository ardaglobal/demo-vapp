...
mod tests {
    ...
    async fn test_7_step_insertion_algorithm_basic(pool: PgPool) {
        ...
        let result = tree.insert_nullifier(100).await.expect("First insertion should succeed");
        
        // Verify basic properties
        assert_ne!(result.old_root, result.new_root, "Root should change after insertion");
        ...
        info!("📊 First insertion metrics: {} hashes, {} range checks, {} DB rounds, {} constraints",
            metrics.hash_operations, metrics.range_checks, metrics.database_rounds, metrics.constraints_count);
        
        // Performance assertions
        assert_eq!(metrics.range_checks, 2, "Should perform exactly 2 range checks");
        assert!(metrics.hash_operations <= 99, "Hash operations should be ≤ 3n+3 = 99 for 32-level tree");
        assert!(metrics.constraints_count <= 300, "Should have ~200 constraints, allowing some margin");
        assert!(metrics.database_rounds <= 10, "Should minimize database round trips");
        
        // Verify proof integrity
        assert!(tree.verify_insertion_proof(&result.insertion_proof, &result.new_root),
    ...
    async fn test_7_step_insertion_sequential(pool: PgPool) {
        ...
        for (i, &value) in values.iter().enumerate() {
            ...
            let result = tree.insert_nullifier(value).await
                .unwrap_or_else(|e| panic!("Failed to insert nullifier {}: {:?}", value, e));
            
            // Verify root progression
            if i > 0 {
                assert_ne!(result.old_root, previous_root, "Should build upon previous state");
            }
            ...
            let metrics = &result.operations_count;
            assert_eq!(metrics.range_checks, 2, "Each insertion should perform exactly 2 range checks");
            assert!(metrics.hash_operations <= 99, "Each insertion should meet hash operation limit");
            
            // Verify proof
            assert!(tree.verify_insertion_proof(&result.insertion_proof, &result.new_root),
                "Proof for insertion {} should verify", i + 1);
                
            info!("✅ Insertion {} complete: {} constraints", i + 1, metrics.constraints_count);
        }
        
        // Verify final tree state
        let stats = tree.db.state.get_stats().await.expect("Should get tree stats");
        assert_eq!(stats.total_nullifiers, values.len() as i64);
        assert_eq!(stats.next_index, values.len() as i64);
        assert!(stats.chain_valid, "Chain should remain valid after all insertions");
    }
    ...
    async fn test_7_step_insertion_out_of_order(pool: PgPool) {
        ...
        for (i, &(value, description)) in insertion_order.iter().enumerate() {
            ...
            assert!(low_before.value < value, "Low nullifier should be less than new value");
            assert!(low_before.next_value == 0 || value < low_before.next_value, 
                "New value should fit in the gap");
            
            // Verify pointer updates
            assert_eq!(low_after.next_value, value, "Low nullifier should now point to new value");
            assert_eq!(low_after.next_index, Some(result.nullifier.tree_index), 
                "Low nullifier should point to new tree index");
            
            // Verify new nullifier inherits old low's pointers
            assert_eq!(result.nullifier.next_value, low_before.next_value,
                "New nullifier should inherit low's next_value");
            assert_eq!(result.nullifier.next_index, low_before.next_index,
        ...
        assert!(tree.db.nullifiers.validate_chain().await.expect("Should validate chain"),
    ...
    async fn test_7_step_range_validation_errors(pool: PgPool) {
        ...
        tree.insert_nullifier(500).await.expect("Initial insertion should succeed");
        ...
        tree.insert_nullifier(800).await.expect("Second insertion should succeed");
    ...
    async fn test_7_step_constraint_counting(pool: PgPool) {
        ...
        for i in 1..=10 {
            ...
            info!("Insertion {}: {} hashes, {} range checks, {} constraints",
            ...
            assert_eq!(metrics.constraints_count, expected_total,
                "Constraint calculation should match expected formula");
            
            // Performance requirements
            assert!(metrics.constraints_count <= 300, 
                "Each insertion should have ≤300 constraints (target ~200)");
            assert_eq!(metrics.range_checks, 2, "Should always perform exactly 2 range checks");
        }
        
        let avg_constraints = total_constraints / 10;
        let avg_hashes = total_hashes / 10;
        
        info!("📊 Averages over 10 insertions: {} constraints, {} hashes", avg_constraints, avg_hashes);
        info!("🎯 Target vs Actual: ~200 constraints (got {}), ≤99 hashes (got {})", 
            avg_constraints, avg_hashes);
        
        // Verify we meet the performance goals from the paper
        assert!(avg_constraints < 1600, "Should be much better than 256-level tree (~1600 constraints)");
        assert!(avg_hashes <= 99, "Should meet 3n+3 hash limit for 32-level tree");
    }
    ...
    async fn test_7_step_proof_verification(pool: PgPool) {
        ...
        for value in values {
            let result = tree.insert_nullifier(value).await.expect("Insertion should succeed");
            results.push(result);
        }
        ...
        for (i, result) in results.iter().enumerate() {
            ...
            assert_eq!(proof.low_nullifier_proof.siblings.len(), 32, 
                "Should have 32 siblings for 32-level tree");
            assert_eq!(proof.new_nullifier_proof.siblings.len(), 32,
                "Should have 32 siblings for 32-level tree");
            
            // Verify proof verification
            assert!(tree.verify_insertion_proof(proof, &result.new_root),
                "Proof {} should verify against its corresponding root", i + 1);
            
            // Verify proof should fail against different root
            if i > 0 {
                assert!(!tree.verify_insertion_proof(proof, &results[i-1].new_root),
                    "Proof {} should not verify against different root", i + 1);
            }
        ...
        assert!(tree.verify_merkle_proof(low_proof, &last_result.new_root),
            "Individual Merkle proof should verify");
    ...
    async fn test_7_step_batch_operations_performance(pool: PgPool) {
        ...
        info!("  - Average time per insertion: {}ms", avg_time_per_insertion);
        info!("  - Average DB rounds per insertion: {:.1}", avg_db_rounds);
        info!("  - Average constraints per insertion: {:.0}", avg_constraints);
        
        // Performance assertions
        assert!(avg_time_per_insertion < 1000, "Should average <1s per insertion");
        assert!(avg_db_rounds <= 10.0, "Should minimize database round trips");
        assert!(avg_constraints <= 300.0, "Should maintain constraint efficiency");
        ...
        assert!(stats.chain_valid, "Chain should remain valid after batch operations");
    ...
    async fn test_7_step_edge_cases(pool: PgPool) {
        ...
        let stats = tree.db.state.get_stats().await.expect("Should get stats");
        assert!(stats.chain_valid, "Chain should remain valid after edge case tests");
    ...
    async fn test_algorithm_step_by_step_verification(pool: PgPool) {
        ...
        let new_value = 300;
        info!("🔍 Manually verifying 7-step algorithm for value {}", new_value);
        
        // Step 1: Find low nullifier (manual verification)
        let low_nullifier = tree.db.nullifiers.find_low_nullifier(new_value).await
            .expect("Should find low nullifier").expect("Low nullifier should exist");
        info!("Step 1 ✅: Found low nullifier with value {}", low_nullifier.value);
        
        // Step 2: Membership check (manual verification)
        let exists = tree.db.nullifiers.exists(low_nullifier.value).await
        ...
        assert!(low_nullifier.next_value == 0 || new_value < low_nullifier.next_value, 
            "Range check 2 should pass");
        info!("Step 3 ✅: Range validation passed");
        
        // Steps 4-7: Execute insertion and verify state changes
        let result = tree.insert_nullifier(new_value).await
            .expect("Algorithm insertion should succeed");
        
        // Verify the low nullifier was updated correctly
        let updated_low = tree.db.nullifiers.get_by_value(low_nullifier.value).await
            .expect("Should retrieve updated low nullifier")
            .expect("Updated low nullifier should exist");
        
        assert_eq!(updated_low.next_value, new_value, "Low nullifier should point to new value");
        assert_eq!(updated_low.next_index, Some(result.nullifier.tree_index), 
        ...
        assert_eq!(result.nullifier.next_value, low_nullifier.next_value,
            "New nullifier should inherit old next_value");
        assert_eq!(result.nullifier.next_index, low_nullifier.next_index,
