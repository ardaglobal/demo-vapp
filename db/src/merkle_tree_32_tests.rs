#[cfg(test)]
mod tests {
    use crate::merkle_tree_32::{BatchUpdate, MerkleTree32};
    ...
    async fn test_32_level_tree_initialization(pool: PgPool) {
        ...
        assert_eq!(
            tree.zero_hashes().len(),
            33,
            "Should have 33 zero hashes (levels 0-32)"
        );

        // Initialize tree state
        tree.initialize()
            .await
            .expect("Tree initialization should succeed");
        ...
        assert_eq!(
            root, expected_root,
            "Initial root should be zero hash for level 32"
        );
    ...
    async fn test_zero_hash_precomputation(pool: PgPool) {
        ...
        assert_eq!(
            zero_hashes[0], expected_level_0,
            "Level 0 zero hash incorrect"
        );
        ...
        for level in 1..=32 {
            ...
            assert_eq!(
                zero_hashes[level], expected,
                "Level {} zero hash incorrect",
                level
            );
        }

        info!(
            "✅ All {} zero hashes computed correctly",
            zero_hashes.len()
        );
    }
    ...
    async fn test_single_leaf_update(pool: PgPool) {
        ...
        let new_root = tree
            .update_leaf(leaf_index, leaf_value)
            .await
        ...
        let db_root = tree
            .get_root()
            .await
            .expect("Should get root from database");
        assert_eq!(
            new_root, db_root,
            "Database root should match returned root"
        );

        info!(
            "✅ Leaf {} updated, new root: {:02x?}",
            leaf_index,
            &new_root[..8]
        );
    }
    ...
    async fn test_proof_generation_and_verification(pool: PgPool) {
        ...
        assert_eq!(
            ...
            "Proof should have correct leaf index"
        );
        assert_eq!(
            proof.proof_hashes.len(),
            32,
            "Proof should have exactly 32 sibling hashes"
        );
        assert_eq!(
            proof.leaf_hash, leaf_value,
            "Proof should include correct leaf hash"
        );
        ...
        assert!(
            proof.verify_with_leaf(&leaf_value, &new_root),
            "Custom leaf verification should succeed"
        );

        let wrong_leaf = [0x99u8; 32];
        assert!(
            !proof.verify_with_leaf(&wrong_leaf, &new_root),
            "Custom leaf verification should fail with wrong leaf"
        );

        info!(
            "✅ Proof verification tests passed, proof size: {} bytes",
            proof.size_bytes()
        );
    }
    ...
    async fn test_batch_update_performance(pool: PgPool) {
        ...
        assert_ne!(
            new_root, initial_root,
            "Root should change after batch update"
        );

        // Verify individual leaves were updated correctly
        for update in &updates {
            let proof = tree
                .generate_proof(update.leaf_index)
                .await
                .expect("Should generate proof for updated leaf");
            assert_eq!(
                proof.leaf_hash, update.new_value,
                "Leaf {} should have correct value",
                update.leaf_index
            );
            assert!(
                proof.verify(&new_root),
        ...
        assert!(
            avg_time_per_update < 10_000,
            "Should average <10ms per update"
        );
        assert!(
            metrics.constraint_count < 1600 * batch_size as u32,
    ...
    async fn test_tree_capacity_limits(pool: PgPool) {
        ...
        assert!(
            proof_result.is_err(),
            "Should reject proof generation for invalid index"
        );
    ...
    async fn test_constraint_optimization(pool: PgPool) {
        ...
        tree.update_leaf(leaf_index, leaf_value)
            .await
            .expect("Leaf update should succeed");
        ...
        let _metrics = tree.calculate_metrics(1);
        ...
        info!(
            "  - Constraint reduction: {}x",
            tree_256_constraints / tree_32_constraints
        );
        info!(
            "  - Proof size: {} bytes vs {} bytes",
            proof.size_bytes(),
            256 * 32
        );
        ...
        assert_eq!(
            proof.proof_hashes.len(),
            32,
            "Should have exactly 32 proof elements"
        );
        assert!(
            tree_32_constraints < tree_256_constraints,
        ...
        info!(
            "✅ Constraint optimization verified: {}x reduction",
            tree_256_constraints / tree_32_constraints
        );
    }
    ...
    async fn test_tree_statistics(pool: PgPool) {
        ...
        assert_eq!(
            stats.root_hash, current_root,
            "Stats root should match current root"
        );

        // Verify zero hash usage tracking
        assert!(
            !stats.zero_hash_usage.is_empty(),
            "Should track zero hash usage"
        );
    ...
    async fn test_concurrent_updates(pool: PgPool) {
        ...
        let root1 = tree
            .update_leaf(leaf1, value1)
            .await
            .expect("First update should succeed");

        let root2 = tree
            .update_leaf(leaf2, value2)
            .await
            .expect("Second update should succeed");

        // Verify both leaves are correctly stored
        let proof1 = tree
            .generate_proof(leaf1)
            .await
            .expect("Should generate proof for first leaf");
        let proof2 = tree
            .generate_proof(leaf2)
            .await
            .expect("Should generate proof for second leaf");

        assert_eq!(
            proof1.leaf_hash, value1,
            "First leaf should have correct value"
        );
        assert_eq!(
            proof2.leaf_hash, value2,
            "Second leaf should have correct value"
        );

        // Verify both proofs against final root
        assert!(
            proof1.verify(&root2),
            "First proof should verify against final root"
        );
        assert!(
            proof2.verify(&root2),
            "Second proof should verify against final root"
        );
    ...
    async fn test_zero_hash_optimization(pool: PgPool) {
        ...
        assert!(
            zero_hash_count > 20,
            "Should use many zero hashes in sparse tree"
        );
    ...
    async fn test_edge_cases_and_error_handling(pool: PgPool) {
        ...
        let proof = tree
            .generate_proof(empty_leaf_index)
            .await
        ...
        let invalid_batch = vec![BatchUpdate {
            leaf_index: 1usize << 32,
            new_value: [0x42u8; 32],
        }];
    ...
    async fn benchmark_tree_operations(pool: PgPool) {
        ...
        for i in 0..num_operations {
            ...
            tree.update_leaf(i * 2, leaf_value)
                .await
        ...
        let start = std::time::Instant::now();
        tree.batch_update(&batch_updates)
            .await
            .expect("Batch update should succeed");
        ...
        let batch_speedup =
            single_update_time.as_millis() as f64 / batch_update_time.as_millis() as f64;
        ...
        assert!(
            avg_single_update_ms < 100.0,
            "Single updates should be <100ms on average"
        );
        assert!(
            avg_proof_gen_ms < 50.0,
            "Proof generation should be <50ms on average"
        );
        assert!(
            batch_speedup > 1.0,
