...
mod tests {
    ...
    use crate::merkle_tree::MerkleTreeDb;
    use crate::test_utils::TestDatabase;
    use tracing::info;
    use tracing_test::traced_test;

    async fn setup_merkle_tree_db() -> (MerkleTreeDb, TestDatabase) {
        let test_db = TestDatabase::new()
            .await
            .expect("Failed to create test database");

        let merkle_db = MerkleTreeDb::new(test_db.pool.clone());
        (merkle_db, test_db)
    }
    ...
    async fn test_nullifier_insertion_basic() {
        let (merkle_db, test_db) = setup_merkle_tree_db().await;

        // Insert first nullifier
        let result1 = merkle_db.insert_nullifier_complete(100).await;
        assert!(
            result1.is_ok(),
            "Failed to insert first nullifier: {:?}",
            result1
        );
        ...
        assert!(
            result2.is_ok(),
            "Failed to insert second nullifier: {:?}",
            result2
        );
    ...
    async fn test_nullifier_insertion_ordering() {
        ...
        for value in values {
            ...
            assert!(
                chain_valid,
                "Chain validation failed after inserting {}",
                value
            );
        }
        ...
        for nullifier in nullifiers {
            assert!(
                nullifier.value > prev_value,
                "Nullifiers not in sorted order"
            );
    ...
    async fn test_tree_state_operations() {
        ...
        let state = merkle_db
            .state
            .increment_nullifier_count(None)
            .await
            .unwrap();
        assert_eq!(state.total_nullifiers, 1);

        // Get tree stats
        let tree_stats = merkle_db.state.get_stats().await.unwrap();
    ...
    async fn test_large_scale_insertion() {
        ...
        for (i, value) in values.iter().enumerate() {
            ...
            assert!(
                result.is_ok(),
                "Failed to insert nullifier {} (iteration {}): {:?}",
                value,
                i,
                result
            );
            ...
            if i % 10 == 9 {
                ...
                assert!(
                    chain_valid,
                    "Chain validation failed after {} insertions",
                    i + 1
                );
            }
        }
    ...
    async fn test_concurrent_insertions() {
        ...
        assert!(
            chain_valid,
            "Chain validation failed after concurrent insertions"
        );

        // Count successful insertions
        let stats = merkle_db.state.get_stats().await.unwrap();
        info!(
            "Successful concurrent insertions: {}",
            stats.total_nullifiers
        );

        // Should have at least some successful insertions
        assert!(
            stats.total_nullifiers > 0,
    ...
    async fn test_edge_cases() {
        ...
        assert!(
            result.is_ok(),
            "Failed to insert min positive value: {:?}",
            result
        );
