...
pub fn setup_db() -> AdsWrap<SimpleTask> {
    let ads_dir = "ADS";
...
pub fn update_db(ads: &mut AdsWrap<SimpleTask>) {
    ...
    for i in 0..10 {
        ...
        for j in 0..2 {
            ...
            let mut k = [0u8; 32];
            let mut v = [1u8; 32];
            k[0] = u8::try_from(i).unwrap();
            k[1] = u8::try_from(j).unwrap();
            for n in 0..5 {
                k[2] = u8::try_from(n).unwrap();
                v[0] = u8::try_from(n).unwrap();
                let kh = hasher::hash(&k[..]);
                let shard_id = u8::try_from(byte0_to_shard_id(kh[0])).unwrap();
