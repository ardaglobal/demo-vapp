...
contract StateManagementTest is Test {
    ...
    bytes32 public constant TEST_VKEY =
        0x00b51cef3572d1a49ae7f4a332221cab31cdb72b131dbf28fb6ab26e15458fe2;
    ...
    function _loadFixtures() internal {
        ...
        // Load PLONK fixture
        string memory plonkJson = vm.readFile(
    ...
    function test_UpdateState_ValidProof_Success() public {
        ...
        emit StateUpdated(
            STATE_ID_1,
            NEW_STATE_1,
            keccak256(groth16Fixture.proof),
            authorizedPoster,
            block.timestamp
        );
        ...
        assertEq(
            storedProof,
            groth16Fixture.proof,
            "Proof should be stored correctly"
        );
        ...
        assertTrue(
            arithmetic.isProofVerified(proofId),
            "Proof should be marked as verified"
        );
    }
    ...
    function test_GetCurrentState_ExistingState_ReturnsCorrectState() public {
        ...
        assertEq(
            currentState,
            NEW_STATE_1,
            "Should return correct current state"
        );
    }
    ...
    function test_GetCurrentState_NonExistentState_ReturnsZero() public {
        ...
        assertEq(
            currentState,
            bytes32(0),
            "Should return zero for non-existent state"
        );
    }
    ...
    function test_GetStoredProof_ExistingProof_ReturnsCorrectProof() public {
        ...
        assertEq(
            storedProof,
            groth16Fixture.proof,
            "Should return correct stored proof"
        );
    }
    ...
    function test_GetStoredResult_ExistingResult_ReturnsCorrectResult() public {
        ...
        assertEq(
            storedResult,
            groth16Fixture.publicValues,
            "Should return correct stored result"
        );
    }
    ...
    function test_BatchUpdateStates_ValidProofs_AllSucceed() public {
        ...
        assertEq(
            ...
            NEW_STATE_1,
            "First state should be updated"
        );
        assertEq(
            arithmetic.getCurrentState(STATE_ID_2),
            NEW_STATE_2,
    ...
    function test_GetProofById_NonExistentProof_ReturnsEmptyAndFalse() public {
        ...
        (bytes memory proof, bool exists) = arithmetic.getProofById(
            fakeProofId
        );
    ...
    function test_IsProofVerified_NonExistentProof_ReturnsFalse() public {
        bytes32 fakeProofId = keccak256("fake-proof");
        assertFalse(
            arithmetic.isProofVerified(fakeProofId),
            "Non-existent proof should not be verified"
        );
    }

    function test_GetVerificationResult_VerifiedProof_ReturnsCorrectData()
        public
    {
        // Store a verified proof
        vm.prank(authorizedPoster);
        ...
        bytes32 proofId = keccak256(groth16Fixture.proof);
        (bool verified, bytes memory result) = arithmetic.getVerificationResult(
            proofId
        );
    ...
    function test_UpdateState_InvalidProof_ReturnsFalse() public {
        ...
        assertEq(
            storedState,
            bytes32(0),
            "State should not be updated with invalid proof"
        );
    }
    ...
    function test_Gas_SingleStateUpdate() public {
        ...
        assertLt(
            gasUsed,
            1_000_000,
            "Single state update should use reasonable gas"
        );
    }
    ...
    function test_Gas_BatchVsSingleUpdates() public {
        ...
        assertLt(
            batchUpdateGas,
            singleUpdateGas,
            "Batch update should be more gas efficient"
        );
    }
    ...
    function test_Integration_CompleteWorkflow() public {
        ...
        assertTrue(
            arithmetic.isProofVerified(proofId1),
            "First proof should be verified"
        );
        assertTrue(
            arithmetic.isProofVerified(proofId2),
            "Second proof should be verified"
        );

        // 4. Check metadata
        Arithmetic.ProofMetadata memory metadata1 = arithmetic.getProofMetadata(
            proofId1
        );
        assertEq(
            metadata1.stateId,
            STATE_ID_1,
            "Metadata should link to correct state"
        );
        assertEq(
            metadata1.submitter,
            authorizedPoster,
            "Metadata should record correct submitter"
        );
    ...
    function test_Integration_StateHistory() public {
        ...
        assertEq(
            arithmetic.getStateHistoryLength(STATE_ID_1),
            2,
            "Should have 2 states in history"
        );
        ...
        assertEq(
            history[0],
            NEW_STATE_1,
            "First history entry should be first state"
        );
        assertEq(
            history[1],
            NEW_STATE_2,
            "Second history entry should be second state"
        );
    }
}
