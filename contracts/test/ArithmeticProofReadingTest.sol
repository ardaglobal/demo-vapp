...
contract ArithmeticProofReadingTest is Test {
    ...
    function testPostStateUpdateStoresMetadata() public {
        ...
        // Post state update
        vm.prank(user1);
        bool success = arithmetic.postStateUpdate(stateId, newState, proof, result);
        assertTrue(success);
        
        // Verify metadata was stored
        Arithmetic.ProofMetadata memory metadata = arithmetic.getProofMetadata(expectedProofId);
    ...
    function testGetProofById() public {
        ...
        // Test getProofById
        (bytes memory retrievedProof, bool exists) = arithmetic.getProofById(proofId);
        assertTrue(exists);
        assertEq(retrievedProof, proof);
        
        // Test non-existent proof
        (bytes memory noProof, bool notExists) = arithmetic.getProofById(keccak256("non-existent"));
    ...
    function testGetProofByStateId() public {
        ...
        (bytes memory retrievedProof, bytes32 proofId) = arithmetic.getProofByStateId(stateId);
        assertEq(retrievedProof, proof);
        assertEq(proofId, keccak256(proof));
        
        // Test non-existent state
        (bytes memory noProof, bytes32 noProofId) = arithmetic.getProofByStateId(keccak256("non-existent"));
        assertEq(noProof.length, 0);
    ...
    function testGetLatestProof() public {
        ...
        vm.prank(user1);
        arithmetic.postStateUpdate(stateId, keccak256("new-state-4a"), proof1, result1);
        ...
        vm.prank(user2);
        arithmetic.postStateUpdate(stateId, keccak256("new-state-4b"), proof2, result2);
        uint256 secondTimestamp = block.timestamp;
        
        // Test getLatestProof
        (bytes memory latestProof, bytes32 proofId, uint256 timestamp) = arithmetic.getLatestProof(stateId);
        ...
        (bytes memory firstProof, bool exists) = arithmetic.getProofById(keccak256(proof1));
        assertTrue(exists);
    ...
    function testProofVerificationStatus() public {
        ...
        (bool verified, bytes memory retrievedResult) = arithmetic.getVerificationResult(proofId);
        assertTrue(verified);
    ...
    function testProofEnumeration() public {
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            vm.prank(user1);
            arithmetic.postStateUpdate(stateIds[i], keccak256(abi.encodePacked("new-state-", i)), proof, result);
        }
        ...
        for (uint256 i = 0; i < 3; i++) {
            (bytes32 indexProofId, bytes memory indexProof) = arithmetic.getProofByIndex(i);
    ...
    function testGetRecentProofs() public {
        ...
        for (uint256 i = 0; i < 5; i++) {
            ...
            vm.prank(user1);
            arithmetic.postStateUpdate(stateId, keccak256(abi.encodePacked("new-state-", i)), proof, result);
        }
        
        // Test getting recent 3 proofs
        (bytes32[] memory recentProofIds, bytes[] memory recentProofs) = arithmetic.getRecentProofs(3);
        ...
        (bytes32[] memory allProofIds, bytes[] memory allProofs) = arithmetic.getRecentProofs(0);
        assertEq(allProofIds.length, 5);
        assertEq(allProofs.length, 5);
        
        // Test limit larger than total
        (bytes32[] memory limitedProofIds, bytes[] memory limitedProofs) = arithmetic.getRecentProofs(10);
    ...
    function testGetProofsByStateId() public {
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            vm.prank(user1);
            arithmetic.postStateUpdate(stateId, keccak256(abi.encodePacked("new-state-multi-", i)), proof, result);
        }
        ...
        for (uint256 i = 0; i < 3; i++) {
            bytes memory expectedProof = abi.encodePacked("test-proof-multi-", i);
    ...
    function testEmptyStateReturns() public {
        ...
        (bytes memory noLatestProof, bytes32 noLatestProofId, uint256 noTimestamp) = arithmetic.getLatestProof(keccak256("non-existent"));
        assertEq(noLatestProof.length, 0);
        assertEq(noLatestProofId, bytes32(0));
        assertEq(noTimestamp, 0);
        
        (bytes32[] memory emptyRecentIds, bytes[] memory emptyRecentProofs) = arithmetic.getRecentProofs(5);
